...
describe('Terrassement et Résumé Fondation', () => {
  it('CAS DE TEST - Déblais, Remblais, Résumé complet', () => {
    ...
    assert.equal(resume.volumes.videFilant.toFixed(5), '15.17035'); // 15.17035 = (23.7315 - 12.062) * 1.3
    assert.equal(resume.volumes.nivellement.toFixed(6), '26.909025'); // 9.65 * 7.15 * 0.30 * 1.3
    assert.equal(resume.volumes.remblais.toFixed(6), '47.008975');
    ...
    assert.equal(resume.materiaux.ciment, 58);
    ...
    assert.equal(resume.materiaux.gravier.toFixed(5), '11.25616');
    ...
    assert.equal(resume.materiaux.sable.toFixed(6), '7.193100');
    ...
    assert.equal(resume.materiaux.eau.toFixed(6), '1386.168125');
